...
import { distributorDomains } from '../../store/distributor'
...
const isInvalid = computed(() => !normalizedDomainName.value || isLimitInvalid.value || isDuplicate.value)
...
<template>
  <v-dialog :model-value="modelValue" max-width="620" persistent @update:model-value="emit('update:modelValue', $event)">
    ...
    <ConfirmDialog v-model="showConfirm" message="Se actualizará la información del dominio." @confirm="save" />
  </v-dialog>
</template>
