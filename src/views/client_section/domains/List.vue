...
const isEditingMailboxFilters = ref(false)
const domains = computed(() => distributorDomains.filter((domain) => domain.clientId === clientProfile.clientId))
const headers = [{ title: 'Dominio', key: 'name' }, { title: 'Expiración', key: 'expirationDate' }, { title: 'Cuentas activas', key: 'capacity', sortable: false }, { title: 'Estado', key: 'status' }, { title: 'Acciones', key: 'actions', sortable: false, align: 'end' }]
...
const filteredDomains = computed(() => {
  ...
  return domains.value.filter((domain) => {
    const matchesStatus = appliedStatus.value === 'TODOS' || domain.status.toUpperCase() === appliedStatus.value.slice(0, -1)
...
function toggleMailboxFilters() { if (isEditingMailboxFilters.value) appliedMailboxStatus.value = mailboxStatusInput.value; isEditingMailboxFilters.value = !isEditingMailboxFilters.value }
</script>

<template><section><div class="mb-6"><p class="text-overline text-secondary font-weight-bold mb-1">CLIENTE</p><h1 class="text-h4 font-weight-bold">Mis dominios</h1><p class="text-body-1 text-medium-emphasis mt-2">Consulta tus dominios y administra sus cuentas de correo.</p></div><v-card border rounded="xl" elevation="0" class="mb-4"><v-card-text class="d-flex flex-column flex-sm-row justify-space-between align-center py-4"><v-select v-model="statusInput" :items="['ACTIVOS', 'INACTIVOS', 'TODOS']" label="Mostrar" variant="underlined" density="compact" hide-details :disabled="!isEditingFilters" class="w-100 mb-3 mb-sm-0" /><v-text-field v-model="searchInput" placeholder="Buscar" append-inner-icon="mdi-magnify" variant="outlined" density="compact" hide-details class="w-100" /></v-card-text><v-sheet :color="isEditingFilters ? 'primary' : 'grey-darken-1'" class="py-2 px-4 text-center cursor-pointer" v-ripple @click="toggleFilters"><span class="text-caption font-weight-bold text-white text-uppercase d-flex align-center justify-center">{{ isEditingFilters ? 'Aplicar filtros' : 'Cambiar filtros' }}<v-icon icon="mdi-filter-variant" size="small" class="ml-1" /></span></v-sheet></v-card><v-card border rounded="xl" elevation="0"><v-data-table :headers="headers" :items="filteredDomains" items-per-page-text="Dominios por página" page-text="{0}-{1} de {2}"><template #item.capacity="{ item }">{{ item.activeMailboxes }} de {{ item.maxActiveMailboxes }}</template><template #item.status="{ item }"><v-chip :color="item.status === 'Activo' ? '#43A047' : '#FFB300'" size="small" variant="tonal">{{ item.status }}</v-chip></template><template #item.actions="{ item }"><v-btn icon="mdi-card-account-details-outline" color="#2962FF" variant="text" size="small" aria-label="Consultar cuentas" @click="openMailboxes(item)" /></template></v-data-table></v-card><v-dialog v-model="mailboxListDialog" max-width="1000" scrollable><v-card v-if="selectedDomain" rounded="xl"><v-card-title class="d-flex justify-space-between align-start pa-6"><div><p class="text-overline text-secondary font-weight-bold mb-1">DOMINIO</p><h2 class="text-h5 font-weight-bold">{{ selectedDomain.name }}</h2><p class="text-body-2 text-medium-emphasis mt-1">{{ selectedDomain.activeMailboxes }} / {{ selectedDomain.maxActiveMailboxes }} cuentas activas</p></div><v-btn icon="mdi-close" variant="text" @click="mailboxListDialog = false" /></v-card-title><v-card-text class="px-6 pb-6"><div class="d-flex justify-space-between align-center mb-4"><h3 class="text-h6 font-weight-bold">Cuentas de correo</h3><v-btn color="#43A047" prepend-icon="mdi-plus" @click="openNewMailbox">Agregar cuenta</v-btn></div><v-card border rounded="lg" elevation="0" class="mb-4"><v-card-text class="d-flex flex-column flex-sm-row justify-space-between align-center py-4"><v-select v-model="mailboxStatusInput" :items="['ACTIVAS', 'INACTIVAS', 'TODAS']" label="Mostrar" variant="underlined" density="compact" hide-details :disabled="!isEditingMailboxFilters" class="w-100 mb-3 mb-sm-0" /><v-text-field v-model="mailboxSearch" placeholder="Buscar" append-inner-icon="mdi-magnify" variant="outlined" density="compact" hide-details class="w-100" /></v-card-text><v-sheet :color="isEditingMailboxFilters ? 'primary' : 'grey-darken-1'" class="py-2 px-4 text-center cursor-pointer" v-ripple @click="toggleMailboxFilters"><span class="text-caption font-weight-bold text-white text-uppercase d-flex align-center justify-center">{{ isEditingMailboxFilters ? 'Aplicar filtros' : 'Cambiar filtros' }}<v-icon icon="mdi-filter-variant" size="small" class="ml-1" /></span></v-sheet></v-card><v-data-table :headers="mailboxHeaders" :items="filteredMailboxes" items-per-page-text="Cuentas por página" page-text="{0}-{1} de {2}"><template #item.status="{ item }"><v-chip color="#43A047" size="small" variant="tonal">{{ item.status }}</v-chip></template><template #item.actions="{ item }"><v-btn icon="mdi-pencil-outline" color="#FFB300" variant="text" size="small" aria-label="Editar cuenta" @click="openEditMailbox(item)" /></template></v-data-table></v-card-text></v-card></v-dialog><MailboxDialog v-model="mailboxFormDialog" :domain="selectedDomain" :mailbox="selectedMailbox" @saved="saved" /></section></template>
