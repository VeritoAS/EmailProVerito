...
function openPayment() {
  if (canPay.value) paymentDialog.value = true
}
...
function confirmPayment() {
  ...
  const nextDate = parseExpirationDate(previousExpiration)
  ...
  const svrTotal = distributorProfile.svrMonthlyCost * payment.capacity * payment.months
  distributorPayments.unshift({
    id: Date.now(),
    domainId: selectedDomain.id,
    paidAt: new Date().toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' }),
    ...
    customerUnitPrice: payment.customerUnitPrice,
    customerTotal,
    svrUnitCost: distributorProfile.svrMonthlyCost,
    svrTotal,
    distributorDifference: customerTotal - svrTotal,
    newExpirationDate: selectedDomain.expirationDate,
  })
  dataRevision.value += 1
  successMessage.value = 'Pago registrado correctamente. La vigencia del dominio fue actualizada.'
...
<template>
  <section v-if="domain">
    <div class="d-flex align-start justify-space-between flex-wrap ga-3 mb-6">
      ...
      <v-btn color="#43A047" prepend-icon="mdi-credit-card-outline" :disabled="!canPay" @click="openPayment">Pagar</v-btn>
    </div>
    ...
    <PaymentDialog v-model="paymentDialog" :domain="domain" @continue="requestPayment" />
    <ConfirmDialog v-model="showPaymentConfirm" :message="pendingPayment ? `Se registrará un pago de ${currency(pendingPayment.customerTotal)} para ${domain.name}.` : ''" @confirm="confirmPayment" />
