...
const filteredUsers = computed(() => adminUsers.value.filter((user) => statusInput.value === 'TODOS' || user.status.toUpperCase() === statusInput.value.slice(0, -1)))
...
<template>
  <section>
    ...
    <v-card border rounded="xl" elevation="0" class="mb-4"><v-card-text class="d-flex flex-column flex-sm-row justify-space-between align-center ga-4 py-4"><v-select v-model="statusInput" :items="['ACTIVAS', 'INACTIVAS', 'PENDIENTES', 'TODOS']" label="Mostrar" variant="underlined" density="compact" hide-details class="w-100 mb-3 mb-sm-0" /><v-text-field v-model="searchInput" placeholder="Buscar" append-inner-icon="mdi-magnify" variant="outlined" density="compact" hide-details class="w-100" /></v-card-text></v-card>
